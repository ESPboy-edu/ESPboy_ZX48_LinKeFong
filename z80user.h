...
void port_out(unsigned short int port,unsigned char value);

#define Z80_READ_BYTE(address, x) (x)=mem_rd((address));

#define Z80_FETCH_BYTE(address, x)  Z80_READ_BYTE((address), (x))

#define Z80_READ_WORD(address, x) (x)=(mem_rd((address)+0)|(mem_rd((address)+1)<<8));

#define Z80_FETCH_WORD(address, x)  Z80_READ_WORD((address), (x))

#define Z80_WRITE_BYTE(address, x)  mem_wr((address),(x));

#define Z80_WRITE_WORD(address, x)  mem_wr((address+0),(unsigned char)(x)); mem_wr((address+1),(unsigned char)((x)>>8));
...
#define Z80_INPUT_BYTE(port, x)   (x)=port_in((port));

#define Z80_OUTPUT_BYTE(port, x)  port_out((port),(x));
